...
foreach ($rResult as $aRow) {
    ...
    for ($i = 0; $i < count($aColumns); $i++) {
        ...
        if ($aColumns[$i] == 'short_name') {
            $_data = '<a href="' . admin_url('geographical/city/index/' . $aRow[$primaryKey]) . '">' . $_data . '</a>';
            $_data .= '<div class="row-options">';
            $_data .= '<a class="view-city" data-city="'.$aRow[$primaryKey].'" href="' . ('view/' . $aRow[$primaryKey]) . '">' . _l('view') . '</a>';
            if($can_edit){

           $_data .= '  <a class="edit-cityadd" href="' . admin_url('geographical/city/cityadd/' . $aRow[$primaryKey]) . '">' . _l('edit') . '</a>';
        
                 
            }
            if ($can_delete) {
                $_data .= ' | <a href="' . admin_url('geographical/city/delete/' . $aRow[$primaryKey]) . '" class="text-danger _delete">' . _l('delete') . '</a>';
            }
            $_data .= '</div>';
